...
<div id="pmpro_level-<?php echo $pmpro_level->id; ?>">
	...
	" method="post">
		...
		<?php if ( $pmpro_review ) : ?>
			<p><?php _e( 'Almost done. Review the membership information and pricing below then <strong>click the "Complete Payment" button</strong> to finish your order.', 'ecademy-toolkit' ); ?></p>
		<?php endif; ?>
		...
		<table id="pmpro_pricing_fields" class="pmpro_checkout" width="100%" cellpadding="0" cellspacing="0" border="0">
			<thead>
				<tr>
					<th>
						<span class="pmpro_thead-name"><?php esc_html_e( 'Membership Level', 'ecademy-toolkit' ); ?></span>
						<?php if ( count( $pmpro_levels ) > 1 ) : ?>
							<span class="pmpro_thead-msg">
								<a href="<?php echo pmpro_url( 'levels' ); ?>"><?php _e( 'change', 'ecademy-toolkit' ); ?></a>
							</span>
						<?php endif; ?>
					</th>
				</tr>
			...
			<tbody>
				<tr>
					<td>
						<p>
							<?php printf( __( 'You have selected the <strong>%s</strong> membership level.', 'ecademy-toolkit' ), $pmpro_level->name ); ?>
						</p>
						...
						<div id="pmpro_level_cost">
							<?php if ( $discount_code && pmpro_checkDiscountCode( $discount_code ) ) : ?>
								<?php printf( __( '<p class="pmpro_level_discount_applied">The <strong>%s</strong> code has been applied to your order.</p>', 'ecademy-toolkit' ), $discount_code ); ?>
							<?php endif; ?>
						...
						<?php if ( $pmpro_show_discount_code ) { ?>
							<?php if ( $discount_code && ! $pmpro_review ) { ?>
								<p id="other_discount_code_p" class="pmpro_small">
									<a id="other_discount_code_a" href="#discount_code"><?php esc_html_e( 'Click here to change your discount code', 'ecademy-toolkit' ); ?></a>.
								</p>
							<?php } elseif ( ! $pmpro_review ) { ?>
								<p id="other_discount_code_p" class="pmpro_small"><?php esc_html_e( 'Do you have a discount code?', 'ecademy-toolkit' ); ?>
									<a id="other_discount_code_a" href="#discount_code"><?php esc_html_e( 'Click here to enter your discount code', 'ecademy-toolkit' ); ?></a>.
								</p>
							<?php } elseif ( $pmpro_review && $discount_code ) { ?>
								<p>
									<strong><?php esc_html_e( 'Discount Code', 'ecademy-toolkit' ); ?>:</strong> <?php echo $discount_code; ?>
								</p>
							<?php } ?>
				...
				<?php if ( $pmpro_show_discount_code ) : ?>
					<tr id="other_discount_code_tr" style="display: none;">
						<td>
							<div>
								<label for="other_discount_code"><?php esc_html_e( 'Discount Code', 'ecademy-toolkit' ); ?></label>
								...
								<input type="button" name="other_discount_code_button" id="other_discount_code_button" value="<?php esc_html_e( 'Apply', 'ecademy-toolkit' ); ?>" />
							</div>
						</td>
					</tr>
				<?php endif; ?>
		...
		<?php if ( ! $skip_account_fields && ! $pmpro_review ) { ?>
			<table id="pmpro_user_fields" class="pmpro_checkout" width="100%" cellpadding="0" cellspacing="0" border="0">
				<thead>
					<tr>
						<th>
							<span class="pmpro_thead-name"><?php esc_html_e( 'Account Information', 'ecademy-toolkit' ); ?></span>
							<span class="pmpro_thead-msg"><?php esc_html_e( 'Already have an account?', 'ecademy-toolkit' ); ?>
								<a href="<?php echo wp_login_url( pmpro_url( 'checkout', '?level=' . $pmpro_level->id ) ); ?>"><?php esc_html_e( 'Log in here', 'ecademy-toolkit' ); ?></a></span>
						</th>
					</tr>
				</thead>
				...
				<tbody>
					<tr>
						<td>
							<div>
								<label for="username"><?php esc_html_e( 'Username', 'ecademy-toolkit' ); ?></label>
							...
							<div>
								<label for="password"><?php esc_html_e( 'Password', 'ecademy-toolkit' ); ?></label>
							...
							if ( $pmpro_checkout_confirm_password ) {
								?>
								<div>
									<label for="password2"><?php esc_html_e( 'Confirm Password', 'ecademy-toolkit' ); ?></label>
							...
							<div>
								<label for="bemail"><?php esc_html_e( 'E-mail Address', 'ecademy-toolkit' ); ?></label>
							...
							if ( $pmpro_checkout_confirm_email ) {
								?>
								<div>
									<label for="bconfirmemail"><?php esc_html_e( 'Confirm E-mail Address', 'ecademy-toolkit' ); ?></label>
							...
							<div class="pmpro_hidden">
								<label for="fullname"><?php esc_html_e( 'Full Name', 'ecademy-toolkit' ); ?></label>
								<input id="fullname" name="fullname" type="text" class="input <?php echo pmpro_getClassForField( 'fullname' ); ?>" size="30" value="" />
								<strong><?php esc_html_e( 'LEAVE THIS BLANK', 'ecademy-toolkit' ); ?></strong>
							</div>
		...
		<?php } elseif ( $current_user->ID && ! $pmpro_review ) { ?>
			<p id="pmpro_account_loggedin">
				<?php printf( __( 'You are logged in as <strong>%1$s</strong>. If you would like to use a different account for this membership, <a href="%2$s">log out now</a>.', 'ecademy-toolkit' ), $current_user->user_login, wp_logout_url( $_SERVER['REQUEST_URI'] ) ); ?>
			</p>
		<?php } ?>
		...
		<?php if ( pmpro_getGateway() == 'paypal' && empty( $pmpro_review ) && true == apply_filters( 'pmpro_include_payment_option_for_paypal', true ) ) { ?>
			...
			<?php if ( ! $pmpro_requirebilling ) { ?>
				...
				<thead>
					<tr>
						<th><?php esc_html_e( 'Choose your Payment Method', 'ecademy-toolkit' ); ?></th>
					</tr>
				</thead>
					...
					<td>
						...
						<span class="gateway_paypal">
							...
							if ( ! $gateway || $gateway == 'paypal' ) {
								?>
								checked="checked"<?php } ?> />
							<a href="javascript:void(0);" class="pmpro_radio"><?php esc_html_e( 'Check Out with a Credit Card Here', 'ecademy-toolkit' ); ?></a>
						</span>
							...
							if ( $gateway == 'paypalexpress' ) {
								?>
								checked="checked"<?php } ?> />
							<a href="javascript:void(0);" class="pmpro_radio"><?php esc_html_e( 'Check Out with PayPal', 'ecademy-toolkit' ); ?></a>
						</span>
						</div>
					</td>
				</tr>
		...
		if ( $pmpro_include_billing_address_fields ) {
			...
			if ( ! $pmpro_requirebilling || apply_filters( 'pmpro_hide_billing_address_fields', false ) ) {
				...
				<thead>
					<tr>
						<th><?php esc_html_e( 'Billing Address', 'ecademy-toolkit' ); ?></th>
					</tr>
				</thead>
				...
				<tr>
					<td>
						<div>
							<label for="bfirstname"><?php esc_html_e( 'First Name', 'ecademy-toolkit' ); ?></label>
						...
						<div>
							<label for="blastname"><?php esc_html_e( 'Last Name', 'ecademy-toolkit' ); ?></label>
						...
						<div>
							<label for="baddress1"><?php esc_html_e( 'Address 1', 'ecademy-toolkit' ); ?></label>
						...
						<div>
							<label for="baddress2"><?php esc_html_e( 'Address 2', 'ecademy-toolkit' ); ?></label>
						...
						if ( $longform_address ) {
							?>
							<div>
								<label for="bcity"><?php esc_html_e( 'City', 'ecademy-toolkit' ); ?></label>
							...
							<div>
								<label for="bstate"><?php esc_html_e( 'State', 'ecademy-toolkit' ); ?></label>
							...
							<div>
								<label for="bzipcode"><?php esc_html_e( 'Postal Code', 'ecademy-toolkit' ); ?></label>
						...
						<?php } else { ?>
							...
							<div>
								<label for="bcity_state_zip"><?php esc_html_e( 'City, State Zip', 'ecademy-toolkit' ); ?></label>
						...
						if ( $show_country ) {
							?>
							<div>
								<label for="bcountry"><?php esc_html_e( 'Country', 'ecademy-toolkit' ); ?></label>
						...
						<div>
							<label for="bphone"><?php esc_html_e( 'Phone', 'ecademy-toolkit' ); ?></label>
						...
						<?php if ( $skip_account_fields ) { ?>
							...
							if ( $pmpro_checkout_confirm_email ) {
								?>
								<div>
									<label for="bconfirmemail"><?php esc_html_e( 'Confirm E-mail', 'ecademy-toolkit' ); ?></label>
									<input id="bconfirmemail" name="bconfirmemail" type="<?php echo( $pmpro_email_field_type ? 'email' : 'text' ); ?>" class="input <?php echo pmpro_getClassForField( 'bconfirmemail' ); ?>" size="30" value="<?php echo esc_attr( $bconfirmemail ); ?>" />
		...
		if ( $pmpro_include_payment_information_fields ) {
			...
			if ( ! $pmpro_requirebilling || apply_filters( 'pmpro_hide_payment_information_fields', false ) ) {
				...
				<thead>
					<tr>
						<th>
							<span class="pmpro_thead-name"><?php esc_html_e( 'Payment Information', 'ecademy-toolkit' ); ?></span>
							<span class="pmpro_thead-msg"><?php printf( __( 'We Accept %s', 'ecademy-toolkit' ), $pmpro_accepted_credit_cards_string ); ?></span>
						</th>
					</tr>
				</thead>
				<tbody>
					...
					<td>
						...
						if ( $pmpro_include_cardtype_field ) {
							...
							<div class="pmpro_payment-card-type">
								<label for="CardType"><?php esc_html_e( 'Card Type', 'ecademy-toolkit' ); ?></label>
						...
						<div class="pmpro_payment-account-number">
							<label for="AccountNumber"><?php esc_html_e( 'Card Number', 'ecademy-toolkit' ); ?></label>
						...
						<div class="pmpro_payment-expiration">
							<label for="ExpirationMonth"><?php esc_html_e( 'Expiration Date', 'ecademy-toolkit' ); ?></label>
						...
						if ( $pmpro_show_cvv ) {
							...
							<div class="pmpro_payment-cvv">
								<label for="CVV"><?php esc_html_e( 'CVV', 'ecademy-toolkit' ); ?></label>
								...
								<small>(<a href="javascript:void(0);" onclick="javascript:window.open('<?php echo pmpro_https_filter( PMPRO_URL ); ?>/pages/popup-cvv.html','cvv','toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=yes, resizable=yes, width=600, height=475');"><?php esc_html_e( "what's this?", 'ecademy-toolkit' ); ?></a>)
								</small>
							</div>
						<?php } ?>
						...
						<?php if ( $pmpro_show_discount_code ) : ?>
							<div class="pmpro_payment-discount-code">
								<label for="discount_code"><?php esc_html_e( 'Discount Code', 'ecademy-toolkit' ); ?></label>
								...
								<input type="button" id="discount_code_button" name="discount_code_button" value="<?php esc_html_e( 'Apply', 'ecademy-toolkit' ); ?>" />
				...
				<tbody>
					<tr class="odd">
						<td>
							...
							<label class="pmpro_normal pmpro_clickable" for="tos"><?php printf( __( 'I agree to the %s', 'ecademy-toolkit' ), $tospage->post_title ); ?></label>
						</td>
					</tr>
				</tbody>
			</table>
		...
		<div class="pmpro_submit">
			<?php if ( $pmpro_review ) { ?>
				<span id="pmpro_submit_span">
					...
					<input type="submit" class="pmpro_btn pmpro_btn-submit-checkout" value="<?php esc_html_e( 'Complete Payment', 'ecademy-toolkit' ); ?> &raquo;" />
				</span>
			<?php } else { ?>
				<?php
				...
				if ( $pmpro_checkout_default_submit_button ) {
					...
					<span id="pmpro_submit_span">
						...
						if ( $pmpro_requirebilling ) {
							esc_attr_e( 'Submit and Check Out', 'ecademy-toolkit' );
						} else {
							esc_attr_e( 'Submit and Confirm', 'ecademy-toolkit' );
						}
						?>
						&raquo;" />
					</span>
			...
			<span id="pmpro_processing_message" style="visibility: hidden;">
				<?php echo apply_filters( 'pmpro_processing_message', esc_html__( 'Processing...', 'ecademy-toolkit' ) ); ?>
			</span>
		</div>
