...
class LP_Certificates_Post_Type {
	...
	public function __construct() {
		...
		add_action( 'manage_' . LP_ADDON_CERTIFICATES_CERT_CPT . '_posts_custom_column',
		...
		add_filter( 'learn-press/review-order/cart-item-subtotal', array( $this, 'lp_cert_cart_item_subtotal' ), 10,
			3 );
	...
	public function lp_cert_cart_item_name( $title, $cart_item, $cart_item_key ) {
		if ( isset( $cart_item['_learnpress_certificate_id'] ) ) {
			$title = sprintf(
				...
				get_the_title( $cart_item['_learnpress_certificate_id'] ),
	...
	public function lp_cert_save_settings( $post_id, $post ) {
		...
		if ( ! isset( $_POST['_lp_certificate_price'] ) || ! wp_verify_nonce( $_POST['certificates_fields'],
	...
	public function row_actions( $actions, $post ) {
		...
		if ( LP_ADDON_CERTIFICATES_CERT_CPT == $post->post_type ) {
			$actions['export_cert'] = sprintf( '<a href="%s">%s</a>',
				...
				if ( $certificate->get_id() ) {
					if ( $preview = $certificate->get_preview() ) {
						...
						echo sprintf( '<a href="%s"><img src="%s" alt="%s" /></a>',
							get_edit_post_link( $certificate_id ), $preview, $certificate->get_name() );
						echo '</div>';
					}
				} else {
					_e( '-', 'ecademy-toolkit' );
			...
			case 'courses':
				...
				if ( $courses = $wpdb->get_results( $query ) ) {
					...
					foreach ( $courses as $course ) {
						$links[] = sprintf( '<a href="%s">%s</a>', get_edit_post_link( $course->ID ),
							$course->post_title );
					}
					$output = join( ' | ', $links );
				}
