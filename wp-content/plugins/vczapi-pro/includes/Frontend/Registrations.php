...
class Registrations {
	...
	public function register_user() {
		...
		//Validation Checks
		if ( empty( $meeting_id ) ) {
			wp_send_json_error( __( 'Meeting ID does not seem to be valid. Please try again later.', 'vczapi-pro' ) );
		}
		...
		if ( ! empty( $registered ) && empty( $registered->code ) ) {
			...
			wp_send_json_success( sprintf( __( 'You have been succesfully registered to this meeting. Please check your email for join links. %s', 'vczapi-pro' ), $link ) );
		} else {
	...
	public function list_author_registrants() {
		...
		wp_enqueue_script( 'vczapi-pro-dt-buttons', VZAPI_ZOOM_PRO_ADDON_DIR_URI . 'assets/vendors/datatables.net-buttons/js/dataTables.buttons.min.js', array(
			'jquery',
			'video-conferencing-with-zoom-api-datable-js'
		), VZAPI_ZOOM_PRO_ADDON_PLUGIN_VERSION, true );
		wp_enqueue_script( 'vczapi-pro-dt-buttons-flash', VZAPI_ZOOM_PRO_ADDON_DIR_URI . 'assets/vendors/datatables.net-buttons/js/buttons.flash.js', array(
			'jquery',
			'video-conferencing-with-zoom-api-datable-js'
		), VZAPI_ZOOM_PRO_ADDON_PLUGIN_VERSION, true );
		wp_enqueue_script( 'vczapi-pro-dt-jszip', VZAPI_ZOOM_PRO_ADDON_DIR_URI . 'assets/vendors/jszip/jszip.min.js', array(
			'jquery',
			'video-conferencing-with-zoom-api-datable-js'
		), VZAPI_ZOOM_PRO_ADDON_PLUGIN_VERSION, true );
		wp_enqueue_script( 'vczapi-pro-dt-html5', VZAPI_ZOOM_PRO_ADDON_DIR_URI . 'assets/vendors/datatables.net-buttons/js/buttons.html5.js', array(
			'jquery',
			'video-conferencing-with-zoom-api-datable-js'
		), VZAPI_ZOOM_PRO_ADDON_PLUGIN_VERSION, true );
		wp_enqueue_script( 'vczapi-pro-dt-print', VZAPI_ZOOM_PRO_ADDON_DIR_URI . 'assets/vendors/datatables.net-buttons/js/buttons.print.js', array(
			'jquery',
			'video-conferencing-with-zoom-api-datable-js'
		), VZAPI_ZOOM_PRO_ADDON_PLUGIN_VERSION, true );
		...
		if ( ! empty( $post_id ) ) {
			...
			if ( ! empty( $meeting_id ) && ! empty( $meeting_details ) && ! empty( $meeting_details['registration'] ) ) {
				...
				if ( ! empty( $registrants ) && ! empty( $registrants->code ) ) {
					...
					$error = __( 'No registrations for this meeting so far.', 'vczapi-pro' );
				} else {
					$error = __( 'No registrations for this meeting so far.', 'vczapi-pro' );
				}
			} else {
				$error = __( 'Registration is not enabled for this meeting.', 'vczapi-pro' );
			}
		} else {
