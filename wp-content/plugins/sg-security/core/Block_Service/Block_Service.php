...
class Block_Service {
	...
	public function change_user_role( $id ) {
		...
		if ( wp_get_current_user()->data->ID === $user_id ) {
			return array(
				'message' => __( 'This will restrict your account to a Subscriber role and will lock you out of Admin Menu', 'sg-security' ),
				'result' => 0,
			);
		}
	...
	public function block_ip( $id, $type ) {
		...
		if ( empty( $ip ) ) {
			return array(
				'message' => __( 'Cloud not find the IP.', 'sg-security' ),
				'result' => 0,
			);
		}
