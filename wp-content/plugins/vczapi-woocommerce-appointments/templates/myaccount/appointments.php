...
if ( ! empty( $tables ) ) : ?>
	...
	<?php foreach ( $tables as $table_id => $table ) : ?>
        ...
        <table class="shop_table shop_table_responsive my_account_orders my_account_appointments <?php echo esc_html( $table_id ) . '_appointments'; ?>">
            <thead>
            <tr>
                <th scope="col" class="appointment-id"><span class="nobr"><?php esc_html_e( 'Appointment1', 'woocommerce-appointments' ); ?></span></th>
			...
			<?php foreach ( $table['appointments'] as $appointment ) : ?>
                ...
                <tr>
                    ...
                    <td class="appointment-when anowrap" data-title="<?php esc_html_e( 'When', 'woocommerce-appointments' ); ?>">
						...
						<?php
						if ( $appointment->get_status() === 'confirmed' || $appointment->get_status() === 'paid' || $appointment->get_status() === 'complete' ) {
							echo '<div class="vczapi-woocommerce-appointment-join-link">';
							echo \Codemanas\ZoomWooCommerceAppointments\Main\DataStore::get_join_link( $appointment );
							echo '</div>';
