...
class WC_Gateway_VentiPay_SCP extends WC_Payment_Gateway
{
  /**
   * API Settings
   */
  ...
  public function __construct()
  {
    ...
    $this->method_title = __('Venti-SBS', 'ventipay');
    $this->method_description = __(
      'Acepta suscripciones en ventipay'
    );
  ...
  public function process_payment($order_id)
  {
    ...
    try {
      ...
      $userMail = get_post_meta($order_id,'sbs_order_email',true);
      $user = get_user_by('login',$userMail);
      $alumno = get_post_meta($order_id,'additional_nom_alum',true).' '.get_post_meta($order_id,'additional_apell_alum',true);
      
      //Obtener productos del carro para poder enviar a ventipay
      $carro = $woocommerce->cart->get_cart();
      $sku = '';
      $nomProducto = '';
      foreach($carro as $items){
        $prod = $items['data'];
        $sku = $prod->get_sku();
        $nomProducto = $prod->get_name();
      }
      switch ($sku) {
        case 'prod_FaRGl57IN3C9bmofOTFbB1um':
          $interval = '1month';
          break;
        case 'prod_jHUdJM8PsjFErsnNPlDfdb8q':
          $interval = '3months';
          break;
        case 'prod_jxpyINInJq7OFPrOXbTQrUUs':
          $interval = '6months';
          break;
      ...
      $create_suscription = wp_remote_post(
        self::API_ENDPOINT . '/subscriptions',
        [
          'headers' => [
          ...
          'body' => wp_json_encode([
            ...
            'currency' => 'clp',
            'trial_days'=>30,
            'interval'=>$interval,//Modificar segun producto
            'name' => $nomProducto, //Colocar nombre de producto
            'proration_behavior'=>'create_prorations',
            'customer_email'=>$userMail, //Colocar el correo del usuario activo
      ...
      if (!is_wp_error($create_suscription)) {
        /**
         ...
         */
        if (isset($new_suscription)
          && !empty($new_suscription->object)
          && 'subscription' === $new_suscription->object
          && !empty($new_suscription->id)
          && !empty($new_suscription->status)
          && 'incomplete' === $new_suscription->status
          && !empty($new_suscription->url))
        {
          /**
           * We add the payment ID to the order metadata
           */
          $order->add_meta_data(
            'ventipay_suscription_id',
            $new_suscription->id,
            true
          );
          update_user_meta($user->ID,'id_cliente_ventipay',$new_suscription->customer_id);
          ...
          return [
            'result' => 'success',
            'redirect' => esc_url_raw($new_suscription->url),
          ];
        }
      }
  ...
  public function ventipay_process_ipn()
  {
    try {
      ...
      if ($order->needs_payment()) {
         ...
         */
        $meta_loan_intent_id = $order->get_meta('ventipay_suscription_id');
        ...
        if (!empty($meta_loan_intent_id)
          && substr($meta_loan_intent_id, 0, 4) === 'sub_'
          ...
          && $meta_loan_intent_id === $posted_loan_intent_id)
        {
          ...
          $retrieve_loan_intent = wp_remote_get(
            self::API_ENDPOINT . '/subscriptions/' . $meta_loan_intent_id,
            [
              'headers' => [
                'Authorization' => 'Basic ' . base64_encode($this->api_key . ':'),
                'Content-Type' => 'application/json',
          ...
          if (empty($retrieved_loan_intent)
            ...
            || 'subscription' !== $retrieved_loan_intent->object
            || 'trialing' !== $retrieved_loan_intent->status
  ...
  {
    $order = wc_get_order($order_id);
    $userMail = get_post_meta($order->get_id(),'sbs_order_email',true);
    if (isset($order) && ($order->get_id())) {
      $meta_loan_intent_id = $order->get_meta('ventipay_suscription_id');
      if (!empty($meta_loan_intent_id)) {
        if ('pending' === $order->get_status()) {
          $message = [
            '<script type="text/javascript">setTimeout(function() { window.location.reload(true); }, 1000);</script>',
        ...
        if ($order->is_paid()) {
          ...
          $resp = wp_remote_post('http://138.68.40.214/api/correoElectronico/correo/token/8hu8fWMCIhCXyq0U4TP0CMJ9waHkCGNcsrqok8zR/format/json',[
            ...
            'timeout' => 45,
            'body' => wp_json_encode([
              'plantilla' => 'Mail-bienvenida-SBS-digital2',
              'correoAsunto' => '¡Bienvenid@ a tu suscripción de Matific con SBS Digital!',
              'correoEmail' => $userMail
            ])

          ]);
          //Guardo la respuesta del correo
          update_post_meta($order->get_id(),'venty_resp_mail_bienvenida',$resp);
          $message = [
