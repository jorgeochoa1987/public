...
use Codemanas\ZoomWooCommerceAppointments\Core\FieldsAbstract;
...
class Fields extends FieldsAbstract {
	 ...
	 * @var string
	 */
	private static $fields_slug = '_vczapi_woocommerce_appointments_';
	/**
	 * Fields constructor.
	 */
	 ...
	 * @param $value
	 */
	public static function set_meta( $post_id, $key, $value = false ) {
	...
	public static function set_cache( $post_id, $key, $value = false, $time = false ) {
		...
		self::set_meta( $post_id, 'cache_exp_' . $key, $time );
		self::set_meta( $post_id, 'cache_' . $key, $value );
	}
	...
	public static function get_cache( $post_id, $key ) {
		...
		if ( $exp > time() ) {
			...
			self::set_meta( $post_id, 'cache_' . $key, '' );
			self::set_meta( $post_id, 'cache_exp_' . $key, '' );
	...
	public static function flush_cache( $post_id, $key ) {
		self::set_meta( $post_id, 'cache_exp_' . $key, false );
		self::set_meta( $post_id, 'cache_' . $key, false );
	}

	/**
	 * @return string
	 */
	public static function getFieldsSlug() {
		return self::$fields_slug;
	}
}