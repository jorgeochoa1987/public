...
<table class="lp-pmpro-membership-list">
				...
				<th class="header-item list-item<?php echo $class_count . ' position-' . $index; ?>">
					...
					<div class="lp-price">
						<?php if ( pmpro_isLevelFree( $level ) ) : ?>
							<?php esc_html_e( 'Free', 'ecademy-toolkit' ); ?>
						<?php else : ?>
							<?php
	...
	<tbody class="lp-pmpro-main">
		<tr class="item-row">
			<td class="list-main item-td item-desc"><?php esc_html_e( 'Number of courses', 'ecademy-toolkit' ); ?></td>
			<?php
			foreach ( $levels as $index => $level ) {
			...
			foreach ( $levels as $index => $level ) :
				...
				<td class="list-item">
					<?php if ( empty( $current_user->membership_level->ID ) || ! $current_level ) { ?>
						<a class="pmpro_btn pmpro_btn-select" href="<?php echo pmpro_url( 'checkout', '?level=' . $level->id, 'https' ); ?>"><?php _e( 'GET IT NOW', 'ecademy-toolkit' ); ?></a>
					<?php } elseif ( $current_level ) { ?>
						<?php
						if ( pmpro_isLevelExpiringSoon( $current_user->membership_level ) && $current_user->membership_level->allow_signups ) {
							?>
							<a class="pmpro_btn pmpro_btn-select"
							href="<?php echo pmpro_url( 'checkout', '?level=' . $level->id, 'https' ); ?>"><?php _e( 'Renew', 'ecademy-toolkit' ); ?></a>
							<?php
						} else {
							?>
							<a class="pmpro_btn disabled" href="<?php echo pmpro_url( 'account' ); ?>"><?php _e( 'Your Level', 'ecademy-toolkit' ); ?></a>
							<?php
						}
						?>
