...
<table class="td" cellspacing="0" cellpadding="6" style="width: 100%; margin:0 0 16px;" border="1">
	...
	if ( is_object( $meeting_details ) ) {
		$join_via_browser = vczapi_get_browser_join_shortcode( $meeting_details->id, $meeting_details->password, true );

		?>
        <tr class="vczapi-join-links-reminder-email">
            <th class="td" scope="row" style="text-align:<?php echo esc_attr( $text_align ); ?>;"><?php esc_html_e( 'Zoom Links', 'woocommerce-appointments' ); ?></th>
            <td class="td" style="text-align:<?php echo esc_attr( $text_align ); ?>;">

                <a href="<?php echo $meeting_details->join_url; ?>"><?php _e( 'Join via App', 'vczapi-woocommerce-appointments' ); ?></a>
                <br/><br/>
                <a href="<?php echo $join_via_browser; ?>"><?php _e( 'Join via Browser', 'vczapi-woocommerce-appointments' ); ?></a>
