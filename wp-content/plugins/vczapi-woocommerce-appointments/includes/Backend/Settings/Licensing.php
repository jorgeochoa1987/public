...
class Licensing {
	...
	public function show_license_form() {
		...
		include_once VCZAPI_WOOCOMMERCE_APPOINTMENTS_DIR . '/includes/Backend/Settings/tpl-licensing.php';
	}
	...
	function save_licensing() {
		...
		if ( ( isset( $_POST['vczapi_woocommerce_appointments_activate'] ) || isset( $_POST['vczapi_woocommerce_appointments_deactivate'] ) ) && ! check_admin_referer( 'vczapi_verify_woocommerce_appointments_nonce', '_vczapi_woocommerce_appointments_licensing_nonce' ) ) {
			return;
		}

		// listen for our activate button to be clicked
		if ( isset( $_POST['vczapi_woocommerce_appointments_activate'] ) ) {
			$this->activate_license();
		}

		// listen for our deactivate button to be clicked
		if ( isset( $_POST['vczapi_woocommerce_appointments_deactivate'] ) ) {
	...
	private function activate_license() {
		...
		$license_field = sanitize_text_field( filter_input( INPUT_POST, 'vczapi_woocommerce_appointments_license_key' ) );
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			if ( is_wp_error( $response ) ) {
				...
				$message = __( 'An error occurred, please try again.', 'vczapi-woocommerce-appointments' );
			}
		} else {
			...
			if ( ! empty( $license_data ) && false === $license_data->success ) {
				switch ( $license_data->error ) {
					case 'expired' :
						$message = sprintf( __( 'Your license key expired on %s. Please check your email for renew notice related to your existing license.', 'vczapi-woocommerce-appointments' ), date_i18n( get_option( 'date_format' ), strtotime( $license_data->expires, current_time( 'timestamp' ) ) ) );
						break;
					case 'disabled' :
					case 'revoked' :
						$message = __( 'Your license key has been disabled.', 'vczapi-woocommerce-appointments' );
						break;
					case 'missing' :
						$message = __( 'Invalid license.', 'vczapi-woocommerce-appointments' );
						break;
					case 'invalid' :
					case 'site_inactive' :
						$message = __( 'Your license is not active for this URL.', 'vczapi-woocommerce-appointments' );
						break;
					case 'item_name_mismatch' :
						$message = sprintf( __( 'This appears to be an invalid license key for %s.', 'vczapi-woocommerce-appointments' ), VCZAPI_ZOOM_WOOCOMMERCE_APPOINTMENTS_PLUGIN_NAME );
						break;
					case 'no_activations_left':
						$message = __( 'Your license key has reached its activation limit.', 'vczapi-woocommerce-appointments' );
						break;
					default :
						$message = __( 'An error occurred, please try again.', 'vczapi-woocommerce-appointments' );
						break;
	...
	function notices() {
		...
		if ( empty( $status ) || $status === "invalid" ) {
			?>
            <div class="error">
                <p><strong><?php echo VCZAPI_ZOOM_WOOCOMMERCE_APPOINTMENTS_PLUGIN_NAME; ?></strong>: Invalid License Key. Add your keys from:
		...
		if ( ! empty( $status ) && $status === "expired" ) {
			...
			?>
            <div class="error">
                <p><strong><?php echo VCZAPI_ZOOM_WOOCOMMERCE_APPOINTMENTS_PLUGIN_NAME; ?></strong>: Your license key has expired. License key is required to receive
