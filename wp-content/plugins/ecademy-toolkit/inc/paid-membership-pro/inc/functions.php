...
function learn_press_pmpro_get_template( $name, $args = null ) {
	if ( file_exists( learn_press_locate_template( $name, 'ecademy-toolkit', LP_PMPRO_TEMPLATE ) ) ) {
...
function learn_press_pmpro_locate_template( $name ) {
	...
	$file = learn_press_locate_template( $name, 'ecademy-toolkit', learn_press_template_path() . '/addons/paid-membership-pro/' );
...
function lp_pmpro_run_mode() {
	$mode = array(
		'auto'       => __( 'Auto', 'ecademy-toolkit' ),
		'normal'     => __( 'Normal', 'ecademy-toolkit' ),
		'background' => __( 'Background', 'ecademy-toolkit' ),
	);
